<script setup>
    ...
    const fetchData = async () => {
        await fetchSocialAssistancesPaginated({ ...serverOptions, ...filters.value})
    }

    const debounceFetchData = debounce(fitchData, 500)
                ...
                <img src="@/assets/images/icons/close-circle-white.svg" class="flex size-6 shrink-0" alt="icon">
                    ...
                    <section id="List-Bantuan-Sosial" class="flex flex-col gap-[14px]">
                        <form id="Page-Search" class="flex items-center justify-between">
                         ...
                         <CardList v-for="socialAssistance in socialAssistances" :key="socialAssistance.id" :item="socialAssistance" v-if="!loading" />
