...
const decrementQuantity = () => {
  ...
  eventParticipant.value.total_price = formatRupiah(
    event.value.price * eventParticipant.value.quantity
  );
};
  ...
  <div class="flex gap-[14px]">
    ...
    >
      ...
      <div class="flex flex-col gap-[14px]">
        <template
          v-for="(participant, index) in event.event_participants?.slice(0, 5)"
          :key="index"
        >
        ...
        <section id="Detail-Payment" class="flex flex-col gap-6 px-6">
          ...
          <div class="flex items-center justify-between">
            ...
            <p class="font-semibold text-lg leading-[22.5px]">
              {{ eventParticipant.quantity }}2x warga
            </p>
          </div>
          <hr class="border-desa-background" />
